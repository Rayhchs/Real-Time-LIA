...
def match(threshold, truths, priors, variances, labels, landms, mask_label, loc_t, conf_t, landm_t, mask_t, idx):
    ...
    conf_mask = mask_label[best_truth_idx]
    conf_mask[best_truth_overlap < threshold] = 0
...
def decode(loc, priors, variances, device='cpu'):
...
def decode_landm(pre, priors, variances, device='cpu'):
...
def match_gious(threshold, truths, priors, variances, labels, landms, loc_t, conf_t, landm_t, idx):
    """Match each prior box with the ground truth box of the highest jaccard
    overlap, encode the bounding boxes, then return the matched indices
    corresponding to both confidence and location preds.
    Args:
